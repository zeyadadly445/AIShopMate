...
import { requireAdminAuth } from '@/lib/admin-auth-db'

export async function GET(request: NextRequest) {
  try {
    // التحقق من صلاحيات المدير باستخدام النظام الجديد
    const adminSession = requireAdminAuth(request)
    if (!adminSession) {
      console.log('❌ Database Admin Auth Failed - No valid session')
      return NextResponse.json({
        error: 'غير مصرح - صلاحيات مدير مطلوبة'
      }, { status: 403 })
    }

    console.log(`📊 جلب بيانات لوحة الإدارة بواسطة: ${adminSession.username} (DB ID: ${adminSession.dbId})`)
    ...
    const dashboardData = {
      ...
      adminSession: {
        ...
        dbId: adminSession.dbId
      }
    }

    console.log('✅ Dashboard data retrieved successfully for database admin')
    return NextResponse.json(dashboardData)

  } catch (error) {
    console.error('💥 خطأ في جلب بيانات لوحة الإدارة (Database):', error)
    return NextResponse.json({
